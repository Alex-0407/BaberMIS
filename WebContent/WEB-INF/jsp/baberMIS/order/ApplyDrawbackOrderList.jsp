...
<body>
	<div class="topbar">
		<div class="leftrace">
			<span class="location">当前位置：</span> <a href="admin/Order/ApplyDrawbackOrderIndex"
				style="color: #333; font-size: 12px;">申请退款订单</a>
		</div>
		<div class="arrow"></div>
		<div class="btnbar">
...
$(document).ready(function(){
	...
	$('#dataTable').datagrid({
		url:"admin/Order/ajaxSearchApplyDrawbackOrder",
		...
		columns:[[
			...
			{field:'operator',title:'操作',width:2,formatter:function(value,row,index){
				return "<a href='javascript:void(0);' onclick=\"applyDrawbackRow("+row.orderId+")\">退款 </a> | <a href='javascript:void(0);' onclick='refuseDrawbackRow("+row.orderId+")'>拒绝退款</a>" ;
			}}
		]],
...
function applyDrawbackRow(orderId){
	$.post("admin/Order/applyDrawback",{orderId:orderId},function(data){
...
function refuseDrawbackRow(orderId){
	$.post("admin/Order/refuseDrawback",{orderId:orderId},function(data){
		$('#dataTable').datagrid("reload");
		$.messager.show({
			title:'提示',
			msg:'您已拒绝退款！',
